function setzen(data, BSlist) {
    ...
    function processBS(BS) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            switch (BS) {
                ...
                case "CleverLunch":
                    BS_result =
                        data[i]["Y"] * Daten.T1 * Daten.T1 - data[i]["AX"];
                    spalte = "AX";
                    break;
                case "Werbung":
                ...
                case "Verpflegungsmehraufwand":
                    BS_result =
                        data[i]["Y"] * Daten.T3 * Daten.T4 - data[i]["BC"];
                    spalte = "BC";
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (data[i]["AF"] > 0) {
            data[i]["AG"] =
                Math.round((data[i]["AF"] / data[i]["Q"]) * 100) / 100;
        }

        if (data[i]["CC"] > 0) {
            data[i]["AD"] =
                ((data[i]["CC"] - data[i]["AG"]) * data[i]["AD"]) / 100;
        }
        console.log("CL", CL);
